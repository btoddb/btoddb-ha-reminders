...
def test_advance_recurring_daily_adds_one_day():
    # start == now: one step puts us strictly past now.
...
def test_advance_recurring_daily_self_heals_after_multi_day_outage():
    # start is 2.5 days in the past — advance_recurring should loop to the first
    # future slot (start + 3 days), not just add one day and leave it still past now.
...
def test_advance_recurring_weekly_self_heals_after_multi_week_outage():
    # start is 2.5 weeks in the past — should advance to the first slot after now.
    ...
    assert nxt is not None
    assert nxt.start > NOW
    # Must be exactly n weeks from old_start
    delta = nxt.start - old_start
    assert delta.seconds == 0  # no sub-day drift
